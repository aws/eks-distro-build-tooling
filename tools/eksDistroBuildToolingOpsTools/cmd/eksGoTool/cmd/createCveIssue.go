...
func init() {
	...
	createCveIssue.Flags().StringP(backportFlag, "b", "", "Create backport issue to all or specified EOL Supported Golang Version(s). All versions by default")
	createCveIssue.Flags().String(announcementSourceUrlFlag, "", "Announcement Source URL e.g. https://groups.google.com/g/golang-announce/c/-hjNw559_tE/m/KlGTfid5CAAJ")
	createCveIssue.Flags().String(announcementBodyFlag, "", "String for the body of the issue created")
	createCveIssue.Flags().String(repositoryFlag, "", "The name of the repository to operate against")
	createCveIssue.Flags().String(repositoryOwnerFlag, "", "Name of the owner of the target GitHub repository")
	createCveIssue.Flags().String(githubEmailFlag, "", "Email associated with the GitHub account")
	createCveIssue.Flags().String(commitBranch, "", "Base branch against which pull requests should be made")
	createCveIssue.Flags().String(authorNameFlag, "", "Author of any commits made by the CLI")
}

func GenerateIssueBody() *string {
	b := strings.Builder{}
	if announcementSourceUrlFlag != "" {
		b.WriteString(fmt.Sprintf("\nFrom %s", announcementSourceUrlFlag))
	}
	if announcementBodyFlag != "" {
		b.WriteString(fmt.Sprintf("\n`%s`", announcementBodyFlag))
	}
	b.WriteString("\n\n")
