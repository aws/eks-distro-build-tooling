...
if ! docker buildx ls | grep $BUILDER_NAME > /dev/null 2>&1; then
    ...
    if [ ${CREATE_BUILDER_PODS:-false} = "true" ]; then
        ...
        docker buildx create \
            ...
            --node=builder-amd64 \
            --driver-opt=namespace=buildkit-testing,nodeselector="arch=AMD64",rootless=true,image=${BUILDKITD_IMAGE:-moby/buildkit:v0.10.1-rootless}
        ...
        docker buildx create \
            ...
            --node=builder-arm64 \
            --driver-opt=namespace=buildkit-testing,nodeselector="arch=ARM64",rootless=true,image=${BUILDKITD_IMAGE:-moby/buildkit:v0.10.1-rootless}
    else
